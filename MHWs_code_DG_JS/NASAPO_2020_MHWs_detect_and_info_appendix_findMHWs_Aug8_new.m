...
elseif isfield(data_in, 'flag_daily_timeseries') && data_in.flag_daily_timeseries
    ...
    if isfield(data_in, 'flag_daily_timeseries_smoothed_percentile') && data_in.flag_daily_timeseries_smoothed_percentile
        ...
        sz_win_left_side = 15; % the window size is sz_win_left_side*2+1
        ...
        for ix=1:size(data_percentile3d,1)
            for iy=1:size(data_percentile3d,2)
                ...
                data_percentile3d_smooth(ix,iy,sz_win_left_side+1:end-sz_win_left_side) = NASAPO_smoothField_TheaS(squeeze(data_percentile3d(ix,iy,:)),'movmean',sz_win);
                ...
                data_percentile3d_smooth(ix,iy,end-sz_win_left_side+1:end) = ...
                    data_percentile3d_smooth(ix,iy,inds_end:inds_end+sz_win_left_side-1);
...
for i = 1:length(data_in.tstep)-delta_tstep+1
    ...
    for j = i+delta_tstep-1:length(data_in.tstep)
        clear bfr*
        % original line before Aug 8 (implementation of 2 days gap within
        % event)
%         bfr_msk=sum(data_in.data(:,:,i:j)>=data_percentile3d(:,:,i:j),3)==length(i:j);
        % after Aug 8
        % 
        bfr_msk= ...
        ...
        if j~=length(data_in.tstep)
            bfr_msk = bfr_msk ...
                ...
                length(i:j) - data_in.num_gap_tsteps_in_event && ...
                data_in.data(:,:,j)<=data_percentile3d(:,:,j) && ...
                data_in.data(:,:,j+1)>data_percentile3d(:,:,j+1) ...
                );
        end
        ...
        if i~=1
            % original line before Aug 8 (implementation of 2 days gap
            % within event
%             bfr_msk_before = sum(data_in.data(:,:,i-1:j)>=data_percentile3d(:,:,i-1:j),3)~=length(i:j)+1;
            % after Aug 8 
            bfr_msk_before = data_in.data(:,:,i-1)<=data_percentile3d(:,:,i-1);
            this needs revisiting to confirm it is enough to check if we are within a previous event with gaps
